...
const headerReaderNavItems: HeaderNavItem[] = [
    { label: "My Desk", href: "/learning-paths", menu: <DropdownMenuMyDesk /> },
];
...
export const HeaderReader = ({ items = headerReaderNavItems, isFullWidth, isFloating, className, user }: HeaderReaderProps) => {
    ...
    return (
        <header
            ref={headerRef}
            className={cx(
                "fixed top-0 z-500 flex h-14 w-full items-center justify-center bg-tertiary md:h-14",
                ...
                >
                        ...
                        <ul className="flex items-center gap-0.5">
                            {items.map((navItem) => (
                                <li key={navItem.label}>
                                    {navItem.menu ? (
                                        <AriaDialogTrigger>
                                            <AriaButton className="flex cursor-pointer items-center gap-0.5 rounded-lg px-1.5 py-1 text-xs font-semibold tracking-[0.075em] uppercase text-primary outline-focus-ring transition duration-100 ease-linear hover:text-brand-secondary_hover aria-expanded:text-brand-secondary focus-visible:outline-2 focus-visible:outline-offset-2">
                    ...
                    <div className="hidden items-center gap-8 md:flex ml-8"> {/* 10px gap between nav menu items and buttons */}
                        ...
                        {user ? (
                            ...
                            <>
                                <Button color="link-color" size={isFloating ? "sm" : "md"} href="/sign-in">
                                    Log in
                                </Button>
                                <Button color="link-color" size={isFloating ? "sm" : "md"} href="/sign-up">
                                    Sign up
                                </Button>
                            </>
                        )}
